...
function Accessoires(props) {
    const accessStock = dataItem.filter(elem=>elem.gender_description==='Accessoires')
    const[accessData,setAccessData] = useState(accessStock)
    ...
    function filterTrueStatus(){
       ...
       let filteredArray =  accessStock.filter(function(element){
            ...
            setAccessData(filteredArray)

        if(trueOnlyArr.length===0 || trueOnlyArr.length===4){
            setAccessData(accessStock)
       }
    }
